...
impl<V: Send + Sync + 'static> QueryCache<V> {
    pub fn new() -> (Self, Arc<RwLock<HashMap<QueryableObjectRef, CachedObject<V>>>>) {
        let objects = Arc::new(RwLock::new(HashMap::new()));
        (Self { objects: objects.clone() }, objects)
    }
    ...
    pub async fn on_entered(
        ...
        let Some(interface) = interfaces.first() else { return };
        ...
        let suggested_bindings = handler
            .suggested_bindings()
            .await
            .unwrap_or_default();

        self.objects.write().await.insert(obj, CachedObject {
            handler,
            spatial,
            field,
            suggested_bindings,
...
impl BeamQueryHandlerHandler for BeamQueryCache {
    async fn intersected(
        ...
        self.0
            .on_entered(obj, field, spatial, interfaces, BeamValue { deepest_point_distance, distance })
            .await;
    }
    ...
    async fn moved(
        ...
        self.0
            .on_value_changed(&obj, BeamValue { deepest_point_distance, distance })
            .await;
    }
...
impl PointsQueryHandlerHandler for PointsQueryCache {
    async fn entered(
        ...
        self.0.on_entered(obj, field, spatial, interfaces, distance).await;
    }
